...
function tagsForPreset(preset: ScriptPreset) {
  ...
  if (preset === "sleep-story") {
    // Conservative: single consistent tag, no random paragraph/sentence tags.
    // Minimises voice-mode switching inside chapters for maximum stability.
    return {
      opener: ["[softly]"],
      positive: ["[softly]"],
      comfort: ["[softly]"],
      amused: ["[softly]"],
      neutral: ["[softly]"],
      paragraphChance: 0,           // Opener is forced for first paragraph; no random tags elsewhere
      sentenceChance: 0,            // No sentence-level tags — maximises voice stability
...
export function applyV3Prosody(opts: {
  ...
  const out = paragraphs.map((p, pi) => {
    ...
    // Sleep-story: always open every chapter with the opener tag for a
    // consistent, low-energy voice entry.  Other presets keep existing behaviour.
    const forceOpener = (pi === 0) && (isNonFirstChapter || preset === "sleep-story");
    const addParagraphTag = forceOpener
