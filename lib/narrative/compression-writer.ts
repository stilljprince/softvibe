...
export async function compressWriteNarrative(
  ...
  const system = [
    ...
    `  1. The causal chain — A leads to B leads to C must remain visible`,
    `  2. Plot progression and character logic, decisions, and the turns those decisions create`,
    `  3. The emotional arc and its turns`,
    `  4. Atmosphere and sensory richness — real, but last: keep the strongest sensory moments and consolidate parallel atmospheric beats first when space must be found; do not let atmosphere survive at the expense of 1 through 3`,
    ...
    ``,
    `━━━ FIGURATIVE LANGUAGE (apply while rewriting) ━━━`,
    `  • Keep metaphors and similes sparse; keep personification rare. Do not use compression as an excuse to add fresh figurative decoration — if a passage already over-uses metaphor, simile, or personification, this is a chance to thin it, not to add more.`,
    `  • Prefer concrete actions and precise nouns/verbs over poetic comparison when combining or trimming scenes.`,
    ``,
    `━━━ HARD CONSTRAINTS ━━━`,
    ...
    `  • Do NOT strip atmosphere entirely — consolidate it, do not remove it — but plot progression and causal clarity outrank it when both cannot fully fit.`,
