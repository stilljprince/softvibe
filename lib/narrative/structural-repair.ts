...
export async function structuralRepairNarrative(
  ...
  const system = [
    ...
    `  1. The causal chain — characters, relationships, settings, and decisions must remain coherent`,
    `  2. Plot progression and premise fulfillment — the repaired story must still deliver what the premise promised`,
    `  3. Character logic and emotional truth`,
    `  4. Atmosphere and sensory texture — real, but last: consolidate where duplicated, never strip outright, and never let it block a structural fix that 1 through 3 require`,
    ...
    ``,
    `━━━ FIGURATIVE LANGUAGE (apply while re-rendering kept beats) ━━━`,
    `  • Keep metaphors and similes sparse; keep personification rare. Re-rendering a surviving beat is not a license to add fresh figurative decoration — spend the freed space on concrete detail, action, and emotional embodiment, not on additional metaphor.`,
    `  • Avoid purple prose and avoid repeating the same sensory inventory across the deepened beat.`,
    ``,
