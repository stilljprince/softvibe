...
import { runMotifDiversityPass } from "./motif-diversity";
...
// Final-supervisor score gate for the conservative Motif Diversity Pass.
// Stories whose best post-supervisor score is at or above this threshold are
// eligible for one fail-soft motif-diversity rewrite — small local edits that
// reduce repeated sensory / object / gesture / closure motifs. Stories below
// this threshold skip the pass entirely (the existing repair / structural
// flow is unchanged). Set strictly above REPAIR_SCORE_THRESHOLD (85) so the
// experiment only touches already-strong stories.
const MOTIF_DIVERSITY_SCORE_THRESHOLD = 86;
...
export async function orchestrateLongformNarrative(
  ...
  if (process.env.SKIP_STORY_SUPERVISOR === "1") {
    ...
    // Conservative Motif Diversity Pass. Runs ONLY for already-strong
    // narratives whose best supervisor score is at or above
    // MOTIF_DIVERSITY_SCORE_THRESHOLD. Stories with bestScore of 85
    // intentionally skip this pass — the existing repair /
    // structural flow remains unchanged for them, and we do not want to
    // risk perturbing borderline drafts with optional surface edits. The
    // pass is fail-soft: on any error, the accepted=no branch keeps
    // finalText unchanged.
    if (!haveSupervisorSignal) {
      // No anchor score — never run the motif pass on an unverified draft.
      console.info(
        `[MOTIF_DIVERSITY] skipped reason=no_supervisor_signal threshold=${MOTIF_DIVERSITY_SCORE_THRESHOLD}`,
      );
    } else if (bestScore < MOTIF_DIVERSITY_SCORE_THRESHOLD) {
      console.info(
        `[MOTIF_DIVERSITY] skipped reason=score_below_threshold score=${bestScore} threshold=${MOTIF_DIVERSITY_SCORE_THRESHOLD}`,
      );
    } else if (process.env.SKIP_MOTIF_DIVERSITY === "1") {
      console.info(
        `[MOTIF_DIVERSITY] skipped reason=kill_switch score=${bestScore} threshold=${MOTIF_DIVERSITY_SCORE_THRESHOLD}`,
      );
    } else {
      try {
        const motifResult = await runMotifDiversityPass({
          finalText,
          outputLanguage: input.outputLanguage,
          wordTarget: totalWordTarget,
          bestScore,
        });
        if (motifResult.accepted) {
          finalText = motifResult.finalText;
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(
          `[MOTIF_DIVERSITY] accepted=no reason=caller_threw:${msg.slice(0, 160)} preset=narrative using=best_text`,
        );
      }
    }
  }
