...
export default function GenerateClient({
  ...
  // Admin-only: generate script and return preview metadata without triggering TTS.
  // Persists the script to job.scriptOverride so a subsequent Generate Audio call
  // uses this exact script.
  async function runScriptPreview() {
    ...
    try {
      const res = await fetch(`/api/jobs/${job.id}/script-preview`, {
        method: "POST",
        credentials: "include",
      });
      const json = await res.json().catch(() => null);
      ...
      const payload =
        json && typeof json === "object" && "data" in (json as object)
          ...
          : json;
      setScriptPreview(payload as typeof scriptPreview);
    } catch {
      setPreviewError("Script-Vorschau fehlgeschlagen.");
    } finally {
    ...
    <SVScene theme={themeKey}>
      ...
      <main style={{ color: themeCfg.uiText, minHeight: "100vh", overflowX: "hidden" }}>
        ...
        >
          ...
          <div style={{ padding: "0 4px" }}>
            ...
            <div style={{ display: "grid", gap: 22 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 4 }}>
                ...
                {isAdmin && job && (
                  <>
                    {/* Step 1: Preview Script — generates script, stops before TTS */}
                    <button
                      type="button"
                      onClick={() => void runScriptPreview()}
                      disabled={job.status !== "QUEUED" || previewLoading}
                      style={{
                        width: "100%",
                        ...
                        color: isDark ? "rgba(199,210,254,1)" : "rgba(55,48,163,1)",
                        border: `1px solid ${isDark ? "rgba(99,102,241,0.4)" : "rgba(99,102,241,0.35)"}`,
                        borderRadius: 999,
                        cursor: job.status !== "QUEUED" || previewLoading ? "not-allowed" : "pointer",
                        opacity: job.status !== "QUEUED" || previewLoading ? 0.5 : 1,
                        transition: "opacity 150ms ease",
                      }}
                    >
                      {previewLoading ? "Script wird generiert…" : "Preview Script"}
                    </button>

                    {/* Step 2: Generate Audio — only enabled after preview, uses stored scriptOverride */}
                    <button
                      type="button"
                      onClick={async () => {
                      ...
                      style={{
                        ...
                        border: `1px solid ${isDark ? "rgba(16,185,129,0.35)" : "rgba(16,185,129,0.3)"}`,
                        borderRadius: 999,
                        cursor: !scriptPreview || job.status === "DONE" ? "not-allowed" : "pointer",
                        opacity: !scriptPreview || job.status === "DONE" ? 0.45 : 1,
                        transition: "opacity 150ms ease",
                      }}
                    >
                      Generate Audio
          ...
          {/* ===== Admin: Script Preview Panel ===== */}
          {isAdmin && (previewError || scriptPreview) && (
            <section style={{ marginTop: 32 }}>
              <h2 style={{ fontSize: "0.72rem", fontWeight: 700, margin: "0 0 12px", color: themeCfg.uiSoftText, letterSpacing: "0.12em", textTransform: "uppercase" }}>
                Script Preview
              </h2>

              {/* Error state */}
              {previewError && !scriptPreview && (
                <div style={{
              ...
              {scriptPreview && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  <div style={{
                    ...
                    {[
                      { label: "Words", value: String(scriptPreview.wordCount) },
                      { label: "Chunks", value: String(scriptPreview.chunkCount) },
                      { label: "Est. duration", value: formatSec(scriptPreview.estimatedDurationSec) },
                      { label: "Language", value: scriptPreview.language.toUpperCase() },
                      { label: "Preset", value: scriptPreview.preset },
                    ].map((item) => (
                      <div key={item.label} style={{
                        ...
                        <div style={{ fontSize: "0.68rem", color: themeCfg.uiSoftText, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 4 }}>
                          {item.label}
                        </div>
                        <div style={{ fontSize: "0.9rem", fontWeight: 700, color: themeCfg.uiText }}>
                          {item.value}
                        </div>
                      </div>
                    ))}
                  ...
                  {/* Chunk breakdown */}
                  {scriptPreview.chunkCount > 1 && (
                    <div style={{
                      padding: "10px 12px", borderRadius: 10,
                      background: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.03)",
                      border: `1px solid ${themeCfg.cardBorder}`,
                      fontSize: "0.78rem", color: themeCfg.uiSoftText,
                    }}>
                      <span style={{ fontWeight: 600, color: themeCfg.uiText }}>Chunk words: </span>
                      {scriptPreview.chunkWordCounts.map((w, i) => `Ch${i + 1}: ${w}w`).join(" · ")}
                    </div>
                  )}
                  ...
                  <div style={{
                    ...
                    <span style={{ fontWeight: 600, color: themeCfg.uiText, fontFamily: "inherit" }}>Voice ID: </span>
                    {scriptPreview.voiceId}
                  </div>

                  {/* TTS pipeline annotation */}
                  <div style={{
                    padding: "10px 14px", borderRadius: 10,
                    background: isDark ? "rgba(251,191,36,0.06)" : "rgba(217,119,6,0.05)",
                    border: `1px solid ${isDark ? "rgba(251,191,36,0.22)" : "rgba(217,119,6,0.2)"}`,
                    fontSize: "0.78rem", color: isDark ? "rgba(251,191,36,0.85)" : "rgba(120,53,15,0.9)",
                    lineHeight: 1.6,
                  }}>
                    <div style={{ fontWeight: 700, marginBottom: 4 }}>TTS transforms (not in stored script):</div>
                    <div>· Chapter 1: story title line is stripped before TTS</div>
                    {scriptPreview.warmupText && (
                      <div>· Chapters 2+: warm-up sentence prepended — <em style={{ fontStyle: "normal", opacity: 0.8 }}>&ldquo;{scriptPreview.warmupText}&rdquo;</em></div>
                    )}
                    <div style={{ marginTop: 6, opacity: 0.75 }}>The stored script above is what will be sent to ElevenLabs (minus these transforms).</div>
                  </div>

                  {/* Script text */}
                  <div>
                    <div style={{ fontSize: "0.72rem", fontWeight: 700, color: themeCfg.uiSoftText, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6 }}>
                      Final Script
                    </div>
                    <textarea
                      readOnly
                      value={scriptPreview.script}
                      rows={18}
                      style={{
                        ...svInputStyle(themeKey, true),
                        resize: "vertical" as const,
